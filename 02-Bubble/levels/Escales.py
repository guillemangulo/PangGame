...
for tmx_file in tmx_files:
    ...
    # Remove the old background and Interactiva layers
    root.findall('layer')
    for layer in root.findall('layer'):
        if layer.get('name') == 'Background' or layer.get('name') == 'Interactive':
            root.remove(layer)
    ...
    # Reorder the layers in descending id order
    root[:] = sorted(root[1:], key=lambda layer: int(layer.get('id')), reverse=True)
